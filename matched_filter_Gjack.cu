...
#include "matched_filter_Gpybind11.h"
    ...
    __global__ void network_corr_no_shared(float* templates, float* sum_square_template,
        ...
        float* cc_mat)
    {
        size_t idx, first_sample_block, first_sample_trace, last_sample_trace;
        size_t s;
        ...
        for (size_t c = 0; c < n_components; c++)
        {
            if (weights[s * n_components + c] != 0.)
            {
                ...
                if (last_sample_trace <= n_samples_data)
                {
                    ...
                    {
                        data_sample = data[data_offset + i + threadIdx.x * step];
                        numerator += data_sample * templates[templates_offset + i];
                            ...
                            cc_out, weights, sum_square_templates)
        {
            ...
            while (t_global < (int)n_templates)
            {
                ...
                for (size_t ch = 0; ch < NCHUNKS; ch++)
                {
                    ...
                    // process
                    cudaMemset(cc_mat_d, 0.0, sizeof_cc_mat); // initialize cc_mat to 0
                    network_corr_no_shared <<<GS, BS>>>(templates_d_t,
